...
export default async function sponge(version, build, res) {
    try {
        ...
        if(!(build in versioninfo.artifacts) && build !== "latest") return res.status(400).json({ error: true, msg: "Invalid Build Number! NOTE: Only The 25 Latest Builds May Be Specified For Sponge." });
        ...
        if(!buildinfo.assets || buildinfo.assets.length === 0) return res.status(500).json({ error: true, msg: "No Assets Available For This Build!" });
